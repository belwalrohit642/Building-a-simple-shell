...
char *get_location(char *command)
{
    ...
    if (path)
    {
        // duplicate the path string -> remember to free up memory for this b/c strdup allocates memory that needs to be freed
        path_copy = strdup(path);

        // get length of the command that was passed
        command_length = strlen(command);

        // let's breakdown the path variable and get all directories available
        ...
        while (path_token != NULL)
        {
            ...
            /*  we added 2 for the slash and null character we will introduce in the full command */

            // to build the path for the command, lets copy the directory path and concat the command to it
